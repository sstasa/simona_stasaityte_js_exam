...
function filterDogOwners(arr) {
  let dogOwners = arr.filter(hasDog);
  return dogOwners;
}

function hasDog(user) {
  if (user.hasDog == true) {
    return true;
  } else {
    return false;
...
let dogOwnersArr = filterDogOwners(users);

console.log(`dogOwnersArr = `, dogOwnersArr);

function filterAdults(arr) {
  let adults = arr.filter(isAdult);
  return adults;
}
...
let adultsArr = filterAdults(users);
